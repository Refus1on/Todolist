...
from bot.tg.models import Message, GetUpdatesResponse, SendMessageResponse
from goals.models import Goals, GoalCategory
...
class Command(BaseCommand):
    ...
    def _get_goals(self, message: Message, tg_user: TgUser) -> SendMessageResponse:
        ...
        """
        query_set: QuerySet = (
            Goals.objects.select_related('user')
            .filter(user_id=tg_user.user.id, category__is_deleted=False)
            .exclude(status=Goals.Status.archived)
        )
    ...
    def _create_goal(
        ...
        """
        Goals.objects.create(user_id=user_id, title=title, category_id=category_id)
